...
export async function exportProgressionToPdf(options: MidiGenerationOptions): Promise<Blob> {
    ...
    for (let i = 0; i < chordDetails.length; i++) {
        ...
        // --- Add Guitar Partition Mode (ASCII tablature) ---
        // Generate a simple ASCII guitar tab for the chord and print it below the keyboard.
        // Uses standard tuning (high E .. low E): E4, B3, G3, D3, A2, E2 (MIDI: 64,59,55,50,45,40)
        const tabY = y + rowHeight + 8;
        doc.setFontSize(12);
        doc.setTextColor(0);
